...
import com.dotview.mapviewutil.markerclusterer.GeoClusterer.GeoCluster;
...
public class ClusterMarker extends Overlay {
	 ...
	 *            shadow flag.
	 */
	protected void drawOverlayBitmap(Canvas canvas, Point arg1,
			Projection proj, byte arg3) {
		cluster_.onNotifyDrawFromMarker();
		// Projection proj = mapView.getProjection();
		Point p = proj.toPixels(center_, null);
		// if( p.x < 0 || p.x > mapView.getWidth() || p.y < 0 || p.y >
		// mapView.getHeight() )
		if (p.x < 0 || p.x > canvas.getWidth() || p.y < 0
	...
	public void draw(Canvas canvas, MapView mapView, boolean shadow) {
		...
		Projection proj = mapView.getProjection();
		Point p = proj.toPixels(center_, null);
		if (p.x < 0 || p.x > mapView.getWidth() || p.y < 0
				|| p.y > mapView.getHeight())
			return;
	...
	public boolean isSigle() {
		return GeoItems_.size() == 1;
	}
 
	@Override
	public boolean onTap(GeoPoint arg0, MapView arg1) {
		if(GeoItems_.size() > 1 && cluster_.center_ == arg0){
