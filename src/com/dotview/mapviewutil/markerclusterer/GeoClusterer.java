...
package com.dotview.mapviewutil.markerclusterer;
...
import com.dotview.mapviewutil.GeoBounds;
...
public class GeoClusterer {
	 ...
	 */
	public GeoClusterer(MapView mapView, List<MarkerBitmap> markerIconBmps,Drawable drawable, float screenDensity){
		mapView_ = mapView;
	...
	public void zoomInFixing(){
		if(selcluster_!=null){
			...
			if(getCurBounds().isInBounds(gpt)){
				...
				//mapView_.getController().zoomInFixing(ppt.x, ppt.y); // BROKEN
				mapView_.getController().zoomIn(); // temporary solution to check if everything works
			}
			else{
	...
	public void onTapCalledFromCluster(GeoCluster caller, boolean isTapped) {
		...
		if(isTapped){
			...
			clearSelect();
			selcluster_ = caller;
			mapView_.getController().setCenter(caller.center_);
			zoomInFixing();
	...
	public class GeoCluster {
		...
		private int zoom_;
		private   CustomItemizedOverlay<CustomOverlayItem> itemizedOverlay_;
		/**
		...
		public void clear() {
			...
			if(itemizedOverlay_!=null){
				List<Overlay> mapOverlays = mapView_.getOverlays();
				if(mapOverlays.contains(itemizedOverlay_)){
					mapOverlays.remove(itemizedOverlay_);
				}
				itemizedOverlay_ = null;
			}
			items_ = null;
		...
		public void redraw(){
			...
			if(clusterMarker_ == null) {
				...
				if(clusterMarker_.isSigle()){
					itemizedOverlay_ = new CustomItemizedOverlay<CustomOverlayItem>(
							drawable_, mapView_);
					GeoItem item = items_.get(0);
					GeoPoint point = item.getLocation();

					CustomOverlayItem overlayItem = new CustomOverlayItem(point, item.getTitle(),
							item.getContent(), "", "");
					itemizedOverlay_.addOverlay(overlayItem);

					mapOverlays.add(itemizedOverlay_);
